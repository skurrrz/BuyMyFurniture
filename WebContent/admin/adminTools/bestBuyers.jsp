...
<html>
	...
	<body>
		...
		try {
			...
			String query= "SELECT distinct t1.bidder, SUM(t1.soldPrice) AS totalSales FROM"
					+ " (SELECT a.auctionID, b.bidder, MAX(b.bid_Amount) AS soldPrice FROM Auctions a"
					+ " LEFT JOIN Bids b ON a.auctionID = b.auctionID LEFT JOIN Items i"
					+ " ON a.auctionID = i.auctionID WHERE a.wasSold LIKE '1' GROUP BY a.auctionID)"
					+ " AS t1 GROUP BY t1.bidder ORDER BY totalSales DESC LIMIT " + numberToDisplay + ";";
			...
			if(res.next()){	
				...
				int i = 1;
				%>
				<h1> Top <%=numberToDisplay%> Best-Buying Buyers</h1>
				<hr>
				<br>
				<%
					while (res.next()) {
						String bidder = res.getString("t1.bidder");
						out.print("<strong>" + i + ". " + bidder + ":</strong> Total amount spent = $" + String.format("%.02f", (res.getDouble("totalSales"))));
						i++;
						%>
						<br>
						<%
			...
			<%	
				...
				%>
				<div class='form'>
				<h1> Top <%=numberToDisplay%> Best-Buying Users</h1>
				<%
