...
<html>
	...
	<body>
		...
		try {
			...
			String query= "SELECT t1.item_category AS itemType, SUM(t1.soldPrice) AS totalSales FROM"
					+ " (SELECT a.auctionID, a.item_category, MAX(b.bid_Amount) AS soldPrice FROM"
					+ " Auctions a LEFT JOIN Bids b ON a.auctionID = b.auctionID WHERE a.wasSold"
					+ " LIKE '1' GROUP BY a.auctionID) AS t1 GROUP BY itemType;";
			...
			if(res.next()){	
				...
				<%
				while (res.next()) {		
				out.print("<strong>" + res.getString("itemType") + ":</strong> $" + String.format("%.02f", (res.getDouble("totalSales"))));
				%>
				<br>
				<%
			}
