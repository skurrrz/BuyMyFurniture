	...
	<body>
		...
		try {
			...
			String query= "SELECT t1.item_category AS itemType, t1.item_manufacturer, SUM(t1.soldPrice) AS totalSales"
					...
					+ " FROM Auctions a LEFT JOIN Bids b ON a.auctionID = b.auctionID LEFT JOIN Items i ON" 
					+ " a.auctionID = i.auctionID WHERE a.wasSold LIKE '1' GROUP BY a.auctionID) AS t1"
					+ " GROUP BY t1.item_category, t1.item_manufacturer;";
					...
					if(res.next()){	
						...
						<%
						while (res.next()) {	
							out.print("<strong>" + res.getString("itemType") + "s manufacturered by " 
							+ res.getString("t1.item_manufacturer") + ":</strong> $" + String.format("%.02f", (res.getDouble("totalSales"))));
							%>
							<br>
							<%
						}
