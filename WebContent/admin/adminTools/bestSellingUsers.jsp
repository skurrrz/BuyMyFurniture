...
<html>
	...
	<body>
		...
		try {
			...
			String query= "SELECT distinct t1.seller, SUM(t1.soldPrice) AS totalEarnings FROM"
					+ " (SELECT a.seller, a.auctionID, MAX(b.bid_Amount) AS soldPrice FROM Auctions a"
					+ " LEFT JOIN Bids b ON a.auctionID = b.auctionID WHERE a.wasSold LIKE '1'"
					+ " GROUP BY a.seller, a.auctionID) AS t1 GROUP BY t1.seller ORDER BY totalEarnings"
					+ " DESC LIMIT " + numberToDisplay + ";";
					...
					if(res.next()){	
						...
						int i = 1;
						%>
						<h1> Top <%=numberToDisplay%> Best-Selling Users</h1>
						<hr>
						<br>
						<%
							while (res.next()) {
								out.print(i + ". " + res.getString("t1.seller") + ", total sales $" 
										+ String.format("%.02f", (res.getDouble("totalEarnings"))));
								%>
								<br>
								<%
								i++;
