...
<!---------------------------------- HOW THIS WORKS -------------------------------------
Step 3D of Item Search (see search/searchForItems.jsp for full details)
...
<body>
	...
	<!---------------------- LAMP PARAMETERS RETRIEVED FROM search/newItemsSearch.jsp  ----------------------->	
	<%
	...
	if (!newItemShape.equals("Any")) {
		queryItemShape = " AND i.table_shape LIKE '" + newItemShape + "'";
	} 
	
	if (!newItemNumLegs.equals("Any")) {
		queryItemNumLegs = " AND i.table_num_legs LIKE '" + newItemNumLegs + "'";
	} 
	...
	try {
		...
		Statement stmt = con.createStatement();
			...
			String searchForItem = "SELECT * FROM Auctions a LEFT JOIN Items i ON a.auctionID = i.auctionID"
									+ " WHERE a.isActive LIKE '1' AND a.item_category LIKE 'table' AND title LIKE '%" + newKeyWord1 + "%'"
									+ " AND title LIKE '%" + newKeyWord2 + "%' AND title LIKE '%" + newKeyWord3 + "%'"
									+ queryItemManufacturer + queryItemCondition + queryItemColor + queryItemMaterial + queryItemShape + queryItemNumLegs
