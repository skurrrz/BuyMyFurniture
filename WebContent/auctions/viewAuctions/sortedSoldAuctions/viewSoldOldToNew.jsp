...
<body>
		...
		try {
			...
			//QUERY: Generate a list of all auctions where isActive = 1, auction startdate ascending
			String query= "SELECT * FROM Auctions a LEFT JOIN Items i ON a.auctionID = i.auctionID"
									...
									+ " BY auctionID) t1 ON a.auctionID = t1.auctionID WHERE a.wasSold LIKE"
									+ " '1' ORDER BY auction_startDate ASC;";	
			...
			<% 	
				if(res.next()) {
					...
					%>
					<h1>Sold Auctions: Sorted Oldest to Newest</h1>
					<hr>	
					<%
			...
			------------------------------------------------------------------------------------------------>				
				...
				<% 					
					...
					<div class='form'>
					<%
					out.print("There are currently no sold auctions.");	
					%>
					</div>
					<%
					con.close();
